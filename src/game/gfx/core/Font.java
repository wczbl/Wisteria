...
public class Font {
	...
	public static final int GREEN_COLOR = PaletteHelper.getColor(-1, 40, 40, 40);
	public static final int RED_COLOR = PaletteHelper.getColor(-1, 400, 400, 400);
	public static final int YELLOW_COLOR = PaletteHelper.getColor(-1, 440, 440, 440);
	...
	public static void drawToBitmap(String msg, Screen screen, int x, int y, int col, Bitmap dst) {
		msg = msg.toUpperCase();
		for (int i = 0; i < msg.length(); i++) {
			...
			if (ix >= 0) {
				int xx = ix % 32;
				int yy = ix / 32;
				BitmapHelper.drawHalfTile(screen.getSprites(), x + i * 8, y, xx * 8, (yStart + yy) * 8, col, 0, dst);
			}
		}
	}
