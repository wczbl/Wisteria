...
public class BitmapHelper {
	...
	public static Bitmap loadTextureFromResources(String name) {
		try {
			BufferedImage image = ImageIO.read(BitmapHelper.class.getResource(name));
			int w = image.getWidth();
			int h = image.getHeight();
	...
	public static void drawHalfTile(Bitmap src, int xOffs, int yOffs, int xo, int yo, int cols, int bits, Bitmap dst) {
		BitmapHelper.scaleDraw(src, 1, xOffs, yOffs, xo, yo, 8, 8, cols, bits, dst);
	}
	...
	public static void scaleDraw(Bitmap src, int scale, int xOffs, int yOffs, int xo, int yo, int w, int h, int cols, int bits, Bitmap dst) {
		...
		for(int y = 0; y < h * scale; y++) {
			...
			for(int x = 0; x < w * scale; x++) {
				...
				int col = cols >> src.getPixels()[xs / scale + xo + (ys / scale + yo) * src.getWidth()] * 8 & 255;
				if(col >= 255) continue;
	...
	public static void drawNormal(Bitmap src, int xOffs, int yOffs, Bitmap dst, int alpha) {
		for(int y = 0; y < src.getHeight(); y++) {
			...
			for(int x = 0; x < src.getWidth(); x++) {
				...
				int col = src.getPixels()[x + y * src.getWidth()];
				if(col == alpha) continue;
				dst.getPixels()[xp + yp * dst.getWidth()] = col;
			}
		}
