...
import game.entity.particle.TextParticle;
...
public class Mob extends Entity {
	...
	public void tick() {
		...
		if(this.level.getTile(this.x >> 4, this.y >> 4) == Tile.lava) hurt(this, 4, this.dir ^ 1);
	...
	protected void doHurt(int dmg, int attackDir) {
		...
		this.level.add(new TextParticle("" + dmg, this.x, this.y, col));
		this.health -= dmg;
	...
	public boolean findStartPos(Level level) {
		...
		if(level.getTile(x, y).mayPass(level, xx, yy, this)) {
			this.x = xx;
			this.y = yy;
			return true;
		}
