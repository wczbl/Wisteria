	...
	public Level(int w, int h) {
		...
		for(int y = 0; y < this.h; y++) {
			for(int x = 0; x < this.w; x++) {
				...
				if(this.random.nextInt(40) == 0) setTile(x, y, Tile.rock, 0);				
				if(this.random.nextInt(3) == 0) setTile(x, y, Tile.water, 0);
				if(this.random.nextInt(10) == 0) setTile(x, y, Tile.sand, 0);
